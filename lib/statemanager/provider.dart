...
String? designation_;
      ...
      if (user != null) {
        await FirebaseFirestore.instance
          ...
          if (documentSnapshot.exists) {
            ...
            profileA_ = data['aProfile']==""?"https://res.cloudinary.com/diecwxxmm/image/upload/v1697383258/zellesolutions%20portal/pcqabxada1qmx2bzo0vk.jpg":data['aProfile'];
            emailA_ = data['email'];
            shift_ = data['shift'];
            ...
            FirebaseFirestore.instance
              ...
              if (documentSnapshot.exists) {
               await doesCollectionExist(db_name, user!.uid, checkDateTime().toString().substring(0, 10));
              } else {
 ...
 await lastStats(yesterDay_month!, shift_.toString(), yesterDay_date!, uid!,db_name!);
...
lastStats(String month , String shift,String date,String uid,String dbName )async{
 await FirebaseFirestore.instance.collection('$dbName').doc(uid).collection(yesterDay_date2!).doc(uid).get().then((DocumentSnapshot documentSnapshot) {
    if(documentSnapshot.exists){
      Map<String, dynamic> data =
                documentSnapshot.data() as Map<String, dynamic>;
      lastShiftStart = data['startShift']!=''? data['startShift'].toString().substring(11,19):"not mentioned";
      lastShiftEnd =data['endShift']!=''? data['endShift'].toString().substring(11,19):"not mentioned";
    }
    else{
...
Future<bool> doesCollectionExist(String? shift ,String uid ,String date) async {
  try {
    ...
    if (document.exists) {
      ...
      if ((data['startShift']!="") && (data['endShift']!="")) {
       ...
       await  checkEnabilityBreakBtn(data['active'].toString(),data['activeStartTime'].toString());
       } else {
 ...
 await (data["$callBreakListDBName"]!=null)? data["$callBreakListDBName"].forEach((item) {
 ...
 await (data["$namazBreakListDBName"]!=null)? data["$namazBreakListDBName"].forEach((item) {
    final List<dynamic> durationParts = item["duration"].split(':').map(int.parse).toList();
    final Duration duration = Duration(
      hours: durationParts[0],
      minutes: durationParts[1],
 ...
 await (data["$lunchBreakListDBName"]!=null)? data["$lunchBreakListDBName"].forEach((item) {
    final List<dynamic> durationParts = item["duration"].split(':').map(int.parse).toList();
    final Duration duration = Duration(
      hours: durationParts[0],
      minutes: durationParts[1],
...
await (data["$casualBreakListDBName"]!=null)? data["$casualBreakListDBName"].forEach((item) {
    final List<dynamic> durationParts = item["duration"].split(':').map(int.parse).toList();
    final Duration duration = Duration(
      hours: durationParts[0],
      minutes: durationParts[1],
 ...
 await (data["$summitBreakListDBName"]!=null)? data["$summitBreakListDBName"].forEach((item) {
    final List<dynamic> durationParts = item["duration"].split(':').map(int.parse).toList();
    final Duration duration = Duration(
      hours: durationParts[0],
      minutes: durationParts[1],
...
yesterdayDateData(){
  ...
  } else if ((yesterdayDate.weekday == DateTime.sunday)&&(shift_=='Day')) {
    yesterdayDate = currentDate.subtract(Duration(days: 2));
  }
  else{
